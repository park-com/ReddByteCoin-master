...
const string strMessageMagic = "ReddByte Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   { 
      nSubsidy = 200 * COIN; return nSubsidy; 
       ...
       else if(pindexBest->nHeight < 45884)
   {
        nSubsidy = 200 * COIN / ( 1 + ( pindexBest->nHeight / 259000 ));
    }
       else if(pindexBest->nHeight > 45883)
   {
        nSubsidy = 50 * COIN / ( 1 + ( pindexBest->nHeight / 259000 ));
    }
	
	if(pindexBest->nHeight > 778250)
   {
        nSubsidy = 8.25 * COIN;
    }
	if(pindexBest->nHeight > 1035250)
   {
        nSubsidy = 4.125 * COIN;
    }
	if(pindexBest->nHeight > 1294250)
   {
        nSubsidy = 2.50 * COIN;
    }
	if(pindexBest->nHeight > 1553250)
   {
        nSubsidy = 2 * COIN;
    }
	if(pindexBest->nHeight > 1812250)
   {
        nSubsidy = 1 * COIN;
    }
	if(pindexBest->nHeight > 2071250)
   {
        nSubsidy = 0.50 * COIN;
    }
	if(pindexBest->nHeight > 2271250)
   {
        nSubsidy = 0.25 * COIN;
    }	
	if(pindexBest->nHeight > 2425300)
   {
        nSubsidy = 0.125 * COIN;
    }
	if(pindexBest->nHeight > 2691230)
   {
        nSubsidy = 0.05 * COIN;
    }	
	if(pindexBest->nHeight > 2971250)
   {
        nSubsidy = 0.025 * COIN;
    }	
	if(pindexBest->nHeight > 3571250)
   {
        nSubsidy = 0.005 * COIN;
    }	
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ReddByte", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "This ReddBye coin 2k0d1a7 time";
        CTransaction txNew;
        txNew.nTime = 1483114875;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
